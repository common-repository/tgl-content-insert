...
var $field_id = jQuery(".self_service #tgl_field_id");
var $field_name = jQuery(".self_service #tgl_field_name");
var $field_birthday = jQuery(".self_service #tgl_field_birthday");
...
jQuery().ready( function()
{
    ...
    if(!bookingId || 0 === bookingId.length ||!hash || 0 === hash.length)
    {
        ...
        loadMyBooking(bookingId, hash);
    }
});

jQuery(".self_service #tgl-login").click(function() {
    var bookingId = $field_id.val();
    var name = $field_name.val();
    var birthday = $field_birthday.val();
    var hash = md5(name + "|" + birthday);

    loadMyBooking(bookingId, hash);
})

function loadMyBooking(bookingId, hash) {
    $fields.hide();
    $spinner.show();
